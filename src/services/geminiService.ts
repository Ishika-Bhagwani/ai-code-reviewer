  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      ...
      responseSchema: {
        type: Type.OBJECT,
        properties: {
          summary: { type: Type.STRING, description: "A 2-sentence overview of code health" },
          score: { type: Type.NUMBER, description: "0-100 code quality score" },
          totalIssues: { type: Type.NUMBER },
          timeComplexity: { type: Type.STRING, description: "Big O notation for time complexity" },
          spaceComplexity: { type: Type.STRING, description: "Big O notation for space complexity" },
          complexitySuggestions: { type: Type.STRING, description: "Suggestions for improving time/space complexity" },
          issues: {
            type: Type.ARRAY,
            items: {
              type: Type.OBJECT,
              properties: {
                ...
                category: { type: Type.STRING, enum: ["Logic", "Performance", "Security", "Style"] },
                severity: { type: Type.STRING, enum: ["High", "Medium", "Low"] },
                label: { type: Type.STRING },
                explanation: { type: Type.STRING },
                suggestion: { type: Type.STRING },
                originalCode: { type: Type.STRING, description: "The exact line or snippet of code from the input that is being flagged" }
              },
              required: ["line", "category", "severity", "label", "explanation", "suggestion", "originalCode"]
            }
          }
        },
        required: ["summary", "score", "totalIssues", "timeComplexity", "spaceComplexity", "complexitySuggestions", "issues"]
      }
    }
  });
